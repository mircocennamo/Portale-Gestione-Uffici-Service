...
public interface UfficioRepository extends JpaRepository<Ufficio, String>, JpaSpecificationExecutor<Ufficio> {
    ...
    @Query(value = "SELECT * FROM SEC_UFFICIO_LEVEL sul " +
            ...
            "AND sul.COD_UFF IN (SELECT * FROM JSON_TABLE(GET_UFF_GER(?4),'$[*]' COLUMNS VAL VARCHAR PATH '$')) " +
            "AND sul.DATA_CAN IS NULL",
            countQuery = "SELECT * FROM SEC_UFFICIO_LEVEL sul " +
                    "WHERE (?1 IS NULL OR sul.COD_UFF LIKE ?1% OR sul.UFFICIO LIKE ?1%) " +
                    "AND (?2 IS NULL OR sul.CODICELUOGOUFFICIO = ?2) " +
                    "AND (?3 IS NULL OR sul.FORZA_POLIZIA = ?3) " +
                    "AND sul.COD_UFF IN (SELECT * FROM JSON_TABLE(GET_UFF_GER(?4),'$[*]' COLUMNS VAL VARCHAR PATH '$')) " +
                    "AND sul.DATA_CAN IS NULL",
            nativeQuery = true)
    Page<Ufficio> getUfficiByFiltroPaginated(String parametroRicerca, String codiceLuogo, Integer codiceForzaPolizia, String ufficioOperatore, Pageable pageable);

    @Query(value = "SELECT * " +
            "FROM SEC_UFFICIO_LEVEL sul " +
    ...
    List<Ufficio> autocompleteByForzaPolizia(String parametroRicerca, Integer forzaPolizia, List<String> ufficiDaEscludere);
    ...
    @Query(value = "SELECT SET_PROGRESSIVO_UFFICIO(?1, ?2) FROM DUAL", nativeQuery = true)
    String getNewCodiceUfficio(String provincia, Integer codiceForzaDiPolizia);

    @Query(value = "SELECT COUNT(*) " +
            "FROM USERS_ORARIO_LAVORO uo " +
