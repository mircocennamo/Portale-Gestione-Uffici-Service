...
public class UfficioController {
    ...
    public ResponseEntity<ResponseDto<List<UfficioDto>>> findAllByParametroAutocomplete(@PathVariable String parametroRicerca,
        ...
        List<UfficioDto> result = ufficioService.findAllByParametroAutocomplete(parametroRicerca.toUpperCase(), idForzaPolizia, codiciUfficioDaEscludere);
    ...
    public ResponseEntity<ResponseDto<Page<UfficioDto>>> searchAndPaginateUffici(@RequestBody @Valid UfficioFilterDto filtro,
        ...
        Page<UfficioDto> result = ufficioService.searchAndPaginate(filtro, ufficioOperatore);
    ...
    @Operation(summary = "API per recuperare la lista di uffici tramite descrizione, forza polizia e provincia")
    @PostMapping("/by-provincia-ente-categoria")
    public ResponseEntity<ResponseDto<Page<UfficioDto>>> getUfficioByProvinciaAndForzaPoliziaAndCategoria(@RequestParam(required = false) String parametroRicerca,
                                                                                              @RequestParam(required = false) List<String> siglaProvincia,
                                                                                              @RequestParam(required = false) List<Integer> idForzaPolizia,
                                                                                              @RequestParam(required = false) List<String> codiceCategoria,
                                                                                              @RequestBody PaginazioneDto paginazione){
        return ResponseEntity.ok(ResponseDto.<Page<UfficioDto>>builder()
                .code(HttpStatus.OK.value())
                .body(ufficioService.getUfficioByProvinciaAndForzaPoliziaAndCategoria(parametroRicerca, siglaProvincia, idForzaPolizia, codiceCategoria, paginazione))
                .build());
    }
